...
# 이전 경로 호환성 유지
IMAGE_STORE_PATH = IMAGE_STORE_PATH_WORKFLOW
...
async def save_images(data: Dict[str, Any] = Body(...)):
    ...
    # 저장 경로 결정 (MSA5: workflow, MSA6: results)
    is_result = data.get("is_result", False)
    lot_wafer = data.get("lot_wafer", "")
    table_name = data.get("table_name", "")
    
    if is_result and table_name and lot_wafer:
        # MSA6 결과 이미지 - 테이블별 폴더에 저장
        ...
        # 파일 경로 구성 (lot_wafer 정보 포함)
        before_filename = f"{lot_wafer}_before.{image_format or 'png'}"
        after_filename = f"{lot_wafer}_after.{image_format or 'png'}"
        
        before_path = os.path.join(table_folder, before_filename)
        after_path = os.path.join(table_folder, after_filename)
    else:
        # MSA5 워크플로우 이미지 - workflow_images 폴더에 저장
        storage_path = IMAGE_STORE_PATH_WORKFLOW
        ...
        # 이미지 저장 경로
        before_path = os.path.join(storage_path, before_filename)
        after_path = os.path.join(storage_path, after_filename)
...
async def delete_image(image_id: str):
    ...
    try:
        ...
        # 파일 삭제 - 두 경로 모두 확인
        before_path_workflow = os.path.join(IMAGE_STORE_PATH_WORKFLOW, image.get("before_image_path", ""))
        after_path_workflow = os.path.join(IMAGE_STORE_PATH_WORKFLOW, image.get("after_image_path", ""))
        before_path_results = os.path.join(IMAGE_STORE_PATH_RESULTS, image.get("before_image_path", ""))
        after_path_results = os.path.join(IMAGE_STORE_PATH_RESULTS, image.get("after_image_path", ""))
        
        if os.path.exists(before_path_workflow):
            os.remove(before_path_workflow)
        elif os.path.exists(before_path_results):
            os.remove(before_path_results)
        
        if os.path.exists(after_path_workflow):
            os.remove(after_path_workflow)
        elif os.path.exists(after_path_results):
            os.remove(after_path_results)
...
async def upload_end_image(
    ...
    try:
        ...
        if is_result and table_name and lot_wafer:
            # MSA6 결과 이미지 - 테이블별 폴더에 저장
            image_storage_dir = IMAGE_STORE_PATH_RESULTS
            table_folder = os.path.join(image_storage_dir, table_name)
            
            # 테이블 폴더가 없으면 생성
            os.makedirs(table_folder, exist_ok=True)
            
            # 파일 경로 구성 (lot_wafer 정보 포함)
            filename = f"{lot_wafer}_after.{image_format}"
            file_path = os.path.join(table_folder, filename)
        else:
            # MSA5 워크플로우 이미지 - workflow_images 폴더에 저장
            image_storage_dir = IMAGE_STORE_PATH_WORKFLOW
        ...
        # 상대 URL 생성
        if is_result and table_name:
            # MSA6 결과 이미지의 경우 경로에 테이블 정보 포함
            file_url = f"/images/results/{table_name}/{os.path.basename(file_path)}"
        else:
            # MSA5 워크플로우 이미지
            file_url = f"/images/workflow/{os.path.basename(file_path)}"
